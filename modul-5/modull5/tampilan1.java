...
public class tampilan1 extends javax.swing.JFrame {
    ...
    public tampilan1() {
        ...
        initComponents();
        conn = koneksi.getConnection();  //Menginisialisasi objek conn dengan memanggil metode getConnection() dari kelas koneksi
        ...
        modelw.addColumn("jabatan"); //Menambahkan kolom ke setiap model untuk mendefinisikan struktur data 
    ...
    private void loadDataK() {
        modelw.setRowCount(0); //// Menghapus semua baris yang ada di model tabel 'modelw' sebelum memuat data baru.
        
        try {
            String sql = "SELECT * FROM karyawan"; //query ambil all data dr karywn 
            PreparedStatement ps = conn.prepareStatement(sql); // Membuat objek 'PreparedStatement' untuk menjalankan query SQL.
            ResultSet rs = ps.executeQuery(); //tersimpan di resulset
            while (rs.next()) {
                modelw.addRow(new Object[]{
                    rs.getInt("ID"),
                    rs.getString("nama"),
                    rs.getString("jabatan"), //mngambl data
                    rs.getString("departemen")
                });
            }
        } catch (SQLException e) {
            System.out.println("Error Save Data" + e.getMessage()); // Menangkap pengecualian SQL jika terjadi kesalahan dan mencetak pesan kesalaha.
        }
    }
        ...
        jPanel14Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel14Layout.createSequentialGroup()
                ...
                .addGroup(jPanel14Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel14Layout.createSequentialGroup()
                        .addGap(20, 20, 20)
                        .addComponent(kembali3, javax.swing.GroupLayout.PREFERRED_SIZE, 110, javax.swing.GroupLayout.PREFERRED_SIZE))))
        );
    ...
    if (Tf_nama.getText().isEmpty() || 
        ...
        JOptionPane.showMessageDialog(this, //Menampilkan pesan peringatan jika salah satu field kosong.
            "Harap isi dulu!", 
            "Peringatan", 
...
//        ps.setInt(1, id);
        ...
        ps.executeUpdate(); // Menjalankan query untuk menyimpan data ke database.
        
        JOptionPane.showMessageDialog(this, "Data berhasil ditambahkan!");
        loadDataP();
        loadComboBoxProyek(); // Memuat ulang ComboBox proyek agar data terbaru tampil.
        } 
        ...
        catch (SQLException e) { // Blok catch untuk menangkap kesalahan SQL selama proses penyimpanan data.
            System.out.println("Error Save Data: " + e.getMessage());
        }
    ...
    private void tambah1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tambah1ActionPerformed
     if (tf_nama.getText().isEmpty() || 
        ...
        try{
         ...
         PreparedStatement ps = conn.prepareStatement(sql);
 // Mengisi placeholder query SQL dengan data dari field teks sekaligus menambah data dari tfnamajabatandepertemen
         ps.setString(1, tf_nama.getText());
         ps.setString(2, tf_jabatan.getText());
          ps.setString(3, tf_departemen.getText());
         ps.executeUpdate(); //// Menjalankan query untuk menyimpan data ke dalam database.
         JOptionPane.showMessageDialog(this, "Data berhasil ditambahkan!");
         loadDataK();
         loadComboBoxKaryawan();
       } catch (SQLException e) {
   ...
   //delete karyawan
    private void hapus1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_hapus1ActionPerformed
        ...
        try  {
            ...
            String sql = "DELETE FROM karyawan WHERE id = ?"; // SQL query untuk menghapus data dari tabel 'karyawan' berdasarkan 'id'.
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, Integer.parseInt(tf_id.getText())); // Mengonversi ID yang berupa teks menjadi integer.
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "Data berhasil dihapus!");
            loadDataK();

        } catch (SQLException e) { // Blok catch untuk menangkap kesalahan SQL selama proses penghapusan data.
            System.out.println("Error Save Data" + e.getMessage());
        }
    ...
    private void update1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_update1ActionPerformed
        ...
        if (tabel_karyawan.getRowCount() == 0) {  // Mengecek apakah tabel karyawan kosong (tidak ada data untuk diperbarui).
        JOptionPane.showMessageDialog(this, "THERE IS NO DATA", "DISCLAIMER !!!", JOptionPane.WARNING_MESSAGE);
            }else
        try { // Jika ID sudah diisi dan tabel karyawan berisi data, lanjutkan ke proses pembaruan.
          String sql = "UPDATE karyawan SET nama = ?, jabatan = ?, departemen = ? WHERE id = ?";
          PreparedStatement ps = conn.prepareStatement(sql);  // Mengisi parameter '?' dalam query SQL dengan data baru dari field input.
          ps.setString(1, tf_nama.getText());
          ps.setString(2, tf_jabatan.getText());
          ps.setString(3, tf_departemen.getText());
          ps.setInt(4,Integer.parseInt(tf_id.getText())); // Mengisi ID karyawan untuk kriteria WHERE
          ps.executeUpdate();
    ...
    if (peran.getText().isEmpty()) {
        ...
        try {
        if (id_karyawan.getSelectedItem() == null || id_proyek.getSelectedItem() == null) { // Mengecek apakah pilihan karyawan dan proyek sudah dipilih.
        ...
        // Mengambil ID karyawan dan ID proyek dari item yang dipilih di combo box.
        ...
        // Memeriksa apakah ID karyawan dan ID proyek valid.
        if (idKaryawan == -1 || idProyek == -1) {
            JOptionPane.showMessageDialog(this, "ID Karyawan atau ID Proyek tidak valid");
            return;
        }
        
         // Menghapus spasi di sekitar input 'peran' dan memeriksa apakah tidak kosong.
        ...
        ps.setString(3, peran.getText().trim()); // Mengisi 'peran' tanpa spasi.
    ...
    private void update2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_update2ActionPerformed
        if (Tf_id.getText().isEmpty()) {  // Memeriksa apakah field ID kosong
        ...
        JOptionPane.showMessageDialog(this, "THERE IS NO DATA", "DISCLAIMER !!!", JOptionPane.WARNING_MESSAGE);
                ...
                try {  
                    ...
                    String proname = Tf_nama.getText().isEmpty() ? rs.getString("nama_proyek") : Tf_nama.getText(); // Mengambil nama proyek dan durasi, menggunakan nilai default jika input kosong
                    ...
                    upps.setString(1, proname); //namakrywn
                    upps.setString(2, duration); //projk
                    upps.setInt(3, Integer.parseInt(Tf_id.getText()));
                    upps.executeUpdate();  // Menjalankan perintah update

                    JOptionPane.showMessageDialog(this, "SUKSES shayy");
                    loadDataP();
            } catch (SQLException e) {
    ...
    private void hapus2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_hapus2ActionPerformed
      if (Tf_id.getText().isEmpty() || // Memeriksa apakah field ID, nama, atau durasi kosong
        Tf_nama.getText().isEmpty() || 
        Tf_durasi.getText().isEmpty()) {
        ...
        try  {
            ...
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, Integer.parseInt(Tf_id.getText())); //ngisiparameter
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "Data berhasil dihapus!");
            loadDataP();
        } catch (SQLException e) {
   ...
   //kembali proyek
    ...
    id_proyek.removeAllItems(); // Menghapus semua item yang ada di combo box
    try {
        String sql = "SELECT id, nama_proyek, durasi_pengerjaan FROM proyek";
        PreparedStatement ps = conn.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            String tampilan = String.format("ID: %d - %s (%s)", // Membuat string tampilan untuk setiap item combo box
                rs.getInt("id"),
                rs.getString("nama_proyek"),
                rs.getString("durasi_pengerjaan")
            );
            id_proyek.addItem(tampilan);// Menambahkan item ke combo box
        }
    } catch (SQLException e) {
   ...
   private int getSelectedId(String textComboBox) {
    if (textComboBox == null) return -1;// Memastikan string tidak null
    try {
        // Mengambil bagian dari string yang berisi ID
        String idPart = textComboBox.substring(textComboBox.indexOf("ID: ") + 4);
        return Integer.parseInt(idPart.substring(0, idPart.indexOf(" -")));// Mengembalikan ID sebagai integer
    } catch (Exception e) { // Menangani kesalahan jika parsing gagal
   ...
   //update trransaksi
    private void update3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_update3ActionPerformed
        ...
        try {
        String sql = "UPDATE transaksi SET peran = ?, id_karyawan=? , id_proyek=? WHERE id_karyawan = ? AND id_proyek = ?";// Menyiapkan query SQL untuk memperbarui peran
        PreparedStatement ps = conn.prepareStatement(sql);
        
        // Mendapatkan ID karyawan dan ID proyek dari item yang dipilih
        int idKaryawan = getSelectedId(id_karyawan.getSelectedItem().toString());
        int idProyek = getSelectedId(id_proyek.getSelectedItem().toString());
        ...
        ps.executeUpdate();  // Mengeksekusi pernyataan untuk memperbarui data
        JOptionPane.showMessageDialog(this, "Data Transaksi berhasil diperbarui");
        loadDataT();// Memuat ulang data transaksi untuk menampilkan perubahan
        } catch (SQLException e) {
